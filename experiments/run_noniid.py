#!/usr/bin/env python3
"""Non-IID Analysis: alpha in {0.05, 0.1, 0.3, 0.5, 1.0, 100(IID)}"""
...
def run_one_alpha(alpha, args, train_ds, test_loader, device):
    ...
    ch_sim = RayleighChannelSimulator(mean_snr_db=10.0, seed=args.seed)
    results = []; cum_bits = 0
    ...
    for rnd in range(args.rounds):
        ...
        for i, k in enumerate(selected):
            ...
            for nm, dat in payload.items():
                ...
                if mode == "rank1_fp32":
                    p = global_model.state_dict()[nm]; W=p.view(p.shape[0],-1); m,n=W.shape
                    cum_bits += m*n+32*(m+n)
        ...
        results.append({"method": f"rc_fedbit_{tag}", "alpha": alpha, "round": rnd+1, "acc": acc, "comm_bits": cum_bits})
...
def main():
    ...
    out_file = "/root/RC-FedBiT/results/noniid_analysis.json"
    os.makedirs("/root/RC-FedBiT/results", exist_ok=True)
    if os.path.exists(out_file):
        with open(out_file) as f: all_results = json.load(f)
    else:
        all_results = []
    tag = results[0]["method"] if results else ""
    all_results = [r for r in all_results if r.get("method") != tag]
