...
class AblationClient:
    ...
    def train(self, global_weights, snr_db, t):
        ...
        opt = torch.optim.SGD(
            ...
            lr=self.config.get("local_lr", 0.01),
            momentum=0.9,
        )
...
class AblationServer:
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if name in global_delta:
                    param.data -= (
                        lr * global_delta[name].to(param.device).view(param.shape)
                    )
        self.round += 1
    ...
    def _aggregate_layer(self, name, updates):
        ...
        for data, w in updates:
            B = data["B"].float()
            if data.get("mode", "binary_only") == "binary_only":
        ...
        if self.level >= 3 and h1s:
            fw = list(ws)
            if name in self.prev_h1:
                prev = self.prev_h1[name]
                aw = [
                    abs(
                        float(
                            torch.dot(h, prev) / (h.norm() * prev.norm() + 1e-8)
                        )
                    )
                    for h in h1s
                ]
                fw = [ws[i] * aw[i] for i in range(len(ws))]
            tw_h = sum(fw) + 1e-8
            h1g = sum(fw[i] * h1s[i] for i in range(len(h1s))) / tw_h
...
def run_ablation(level, tag, args, global_model, client_loaders, test_loader, device, rng):
    ...
    for rnd in range(args.rounds):
        ...
        results.append(
            {"method": tag, "round": rnd + 1, "acc": acc, "comm_bits": cum_bits}
        )
        print(
            f"[{tag}] Round {rnd+1}/{args.rounds} | Acc: {acc:.2f}% | CumBits: {cum_bits:.2e}",
            flush=True,
        )
...
def main():
    ...
    # Support both --level (int) and --variant (A0..A4) arguments
    p.add_argument("--level", type=int, choices=[0, 1, 2, 3, 4], default=None)
    p.add_argument("--variant", type=str, choices=["A0", "A1", "A2", "A3", "A4"], default=None)
    ...
    # Resolve level from variant or level arg
    if args.variant is not None:
        level = int(args.variant[1])
    elif args.level is not None:
        level = args.level
    else:
        p.error("Must specify --level or --variant")
    ...
    tag = tags[level]
    print(f"Ablation {tag} (level={level}), Rounds={args.rounds}", flush=True)
    ...
    results = run_ablation(
        level, tag, args, global_model, client_loaders, test_loader, DEVICE, rng
    )
    elapsed = time.time() - t0
    print(
        f"Done in {elapsed/3600:.2f}h | Final acc: {results[-1]['acc']:.2f}%",
        flush=True,
    )
